...
class DepartPickerVC: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    // 피커 뷰의 선택값을 반환하는 연산 프로퍼티를 정의한다. 현재 피커 뷰에 선택값을 반환하는 연산 프로퍼티 정의
    ...
    override func viewDidLoad() {
        ...
        // 2. 피커 뷰 객체를 초기화하고, 최상위 뷰의 서브뷰로 추가한다.
        ...
        // 3. 외부에서 뷰 컨트롤러를 참조할 때를 위한 사이즈를 저장한다.
    ...
    // 문자열 대신 뷰 객체를 반환하기 때문에, 원하는 형태로 레이아웃과 속성을 설정 가능
